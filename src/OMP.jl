...
`dress_fit` parallelises edges within each iteration.
`delta_dress_fit` parallelises the outer subgraph loop.
"""
module OMP

using Libdl

using ..DRESS: DRESSResult, DeltaDRESSResult, HistogramEntry,
...
function _ensure_lib()
    ...
    lib = _CPU_LIB[]
    _FN_INIT[]    = dlsym(lib, :init_dress_graph)
    _FN_FIT[]     = dlsym(lib, :dress_fit_omp)
    _FN_FREE[]    = dlsym(lib, :free_dress_graph)
    _FN_DELTA_STRIDED[] = dlsym(lib, :delta_dress_fit_omp_strided)
end
...
OpenMP-parallel DRESS fitting (edge-parallel).  Same signature as `DRESS.dress_fit()`.
"""
...
OpenMP-parallel Δ^k-DRESS (subgraph-parallel).  Same signature as `DRESS.delta_dress_fit()`.
"""
